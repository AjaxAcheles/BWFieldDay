...
with app.app_context():
    fetch_volunteer_data.create_events_table()
    fetch_volunteer_data.create_roles_table()
    fetch_volunteer_data.create_positions_table()
...
def event_manager():
    if request.method == 'POST':
        ...
        for key, value in form_data.items():
            ...
            if parts[0] == 'event':
                ...
                if parts[1].startswith('new-event'):
                    if value.strip() != "":
                        fetch_volunteer_data.add_event(value.strip())
                else:
                    event_id = parts[1]
                    fetch_volunteer_data.update_event(event_id, value.strip())
            elif parts[0] == 'role':
                # parts[1] = event_id, parts[2] = role id (or new role identifier)
                if parts[2].startswith('new-role'):
                    if value.strip() != "":
                        event_id = parts[1]
                        fetch_volunteer_data.add_role(event_id, value.strip())
                else:
                    role_id = parts[2]
                    fetch_volunteer_data.update_role(role_id, value.strip())
            elif parts[0] == 'position':
                # parts[1] = event_id, parts[2] = role_id, parts[3] = position id (or new position identifier)
                if parts[3].startswith('new-position'):
                    if value.strip() != "":
                        role_id = parts[2]
                        fetch_volunteer_data.add_position(role_id, value.strip())
                else:
                    position_id = parts[3]
                    fetch_volunteer_data.update_position(position_id, value.strip())
        ...
        return render_template('admin_event_management.html', events=fetch_volunteer_data.get_all_events_nested(), predefined_position_options=["Volunteer A", "Volunteer B", "Volunteer C"])
    ...
    if request.method == 'POST':
        ...
        for key, value in request.form.items():
            if key.startswith('volunteer--'):
                ...
                fetch_volunteer_data.update_position_volunteer(position_id, value.strip())
        return redirect(url_for('volunteering'))
    # GET: load the current events (with nested roles and positions)
    events = fetch_volunteer_data.get_all_events_nested()
